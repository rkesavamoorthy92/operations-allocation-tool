...
class FoundationTestCase(unittest.TestCase):
    ...
    def test_snapshot_is_immutable_and_retains_original_configuration(self) -> None:
        ...
        snapshot = self.service.freeze_setup(run_id=run.run_id, program_configuration=configuration, sampling={"method": "percentage", "value": "3"}, random_seed="42", associates=[{"associate_id": "A001", "name": "Alice", "email": "alice@example.test", "active": True, "target": 5, "maximum_capacity": 10, "experience": "new"}])
    ...
    def test_associate_master_and_invalid_snapshot_capacity(self) -> None:
        ...
        with self.assertRaises(InvalidAssociateConfigurationError):
                self.service.freeze_setup(run_id=run.run_id, program_configuration=valid_config(), sampling={}, random_seed=None, associates=[{"associate_id": "A001", "name": "Alice", "email": "alice@example.test", "active": False, "target": 8, "maximum_capacity": 4}])
    ...
    def test_snapshot_associates_require_a_valid_email(self) -> None:
        """Regression test: Outlook Draft Generation (PROJECT_SPEC.md
        section 18) needs a recipient address for each associate, so a
        missing or malformed email must be rejected at setup time rather
        than surfacing as a confusing failure much later during draft
        creation."""
        run = self.service.create_run(program_id="MX-PT", created_by="user", created_on=date(2026, 8, 15))
        with self.assertRaises(InvalidAssociateConfigurationError):
            self.service.freeze_setup(run_id=run.run_id, program_configuration=valid_config(), sampling={}, random_seed=None, associates=[{"associate_id": "A001", "name": "Alice", "active": True, "target": 5, "maximum_capacity": 5}])
        with self.assertRaises(InvalidAssociateConfigurationError):
            self.service.freeze_setup(run_id=run.run_id, program_configuration=valid_config(), sampling={}, random_seed=None, associates=[{"associate_id": "A001", "name": "Alice", "email": "not-an-email", "active": True, "target": 5, "maximum_capacity": 5}])
